    ...
    return (
      ...
      >
        ...
        {!featured && !hideDate && (
          ...
          {/* PUBLICATION */}
          {item.publication && (
            <div className={`text-xs font-black tracking-wider ${item.publication === 'Sport Beach' ? 'normal-case' : 'uppercase'} mb-3 ${textColorClass} opacity-80 ${hoverTextClass} transition-colors`} aria-label={`Published in ${item.publication}`}>
              {item.publication}
            </div>
          )}
        ...
        {isClickable && (
          <div className="pt-2">
            <span className={`font-black underline text-xs sm:text-sm transition-colors inline-flex items-center gap-1 focus:underline ${textColorClass} ${hoverTextClass}`}>
              {item.ctaLabel || (hasVideo ? 'Watch Video' : 'Learn More')}

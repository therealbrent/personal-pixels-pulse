...
Object.entries(pages).forEach(([route, meta]) => {
  ...
  // Add/replace Open Graph meta tags before the closing head tag
  const ogTags = `<meta property="og:title" content="${meta.ogTitle}">
  ...
  // Insert OG tags right before the closing </head> tag for reliability
  html = html.replace(
    /<\/head>/,
    ogTags + '</head>'
  );
